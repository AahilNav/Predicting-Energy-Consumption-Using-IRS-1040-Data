...
chi_all = chicago_all %>% filter(BUILDING_SUBTYPE=="All")
chi_singlefam= chicago_all %>% filter(BUILDING_SUBTYPE=="Single Family")

# delete the missing value rows and abnormal rows
...
#Qi can you recheck and make sure its right? I believe it is: I'm only added a WINTER column so all shifted by one
#Lets also eliminate a lot more of these variables for collinearity
#Lets go over variables again, and make sure I aggregated correctly.  Something is off.
cleaned_all = cleaned_all[,-c(1,2,3,6,7,8,9,10,11,12,13,15,16,17,18,19,20,22,29,30)]
cleaned_fam = cleaned_fam[,-c(1,2,3,6,7,8,9,10,11,12,13,15,16,17,18,19,20,22,29,30)]


# seperate two target variables: THERMS.SQFT.MEAN.2010 and KWH.MEAN.2010
# NOTE: THERMS.SQFT. is not therm heating/squarefootage.  We should honestly do WINTER/SQFT*(OCCUPIED HOUSING/HOUSING)
